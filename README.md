# 26.Controls
Здесь есть код для pause, resume animation. Можно поставить на паузу только layer, т.е. все анимации сразу, которые 
применены к layer. Если нужно сохранить позицию, поставить на паузу и воспроизвести только анимацию из нескольких примененных, то это легко сделать только для нециклических анимаций, таких как: перемещение в рандомные точки, рандомная последовательность масштабов. Исключение - поворот. А вот если надо переместить, например по Х из 0 в 20, потом в 100, потом в 0, то это сложно.
Также важный код для изменения скорости анимации во время анимации без рывков. beginTime=currentTime; offsetTime=currentTime;
